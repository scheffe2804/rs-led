...
20. **Interne Prüfungen durch Georg vollständig und im Dashboard sichtbar nachverfolgen.** Wird ein Erstkontakt zur Ansicht an Georg gegeben, zunächst nur den internen Entwurf dokumentieren. Einen tatsächlichen Versand erst nach Fund im Gesendet-Ordner festhalten. Versandzeit, Empfänger, Betreff und Nachrichten-ID gehören in den bestehenden Twenty-Kundenvorgang. Nach dem verifizierten Prüfversand dieselbe konkrete, Christian zugewiesene Aufgabe sofort mit Status `TODO`, dem Titelpräfix `Georg-Antwort offen ·` und dem verifizierten Versandzeitpunkt als Frist fortführen. Dadurch bleibt sie bis zu Georgs Antwort in Christians Dashboard `Meine TODO-Aufgaben` sichtbar. Georgs Antwort anschließend ebenfalls im Postfach verifizieren und mit Antwortzeit, Nachrichten-ID sowie vollständigem Freigabe- oder Änderungsinhalt in Twenty dokumentieren. Solange keine Antwort vorliegt, ausdrücklich `Antwort ausstehend` festhalten und niemals eine Freigabe unterstellen. Nach der verifizierten Antwort dieselbe Aufgabe auf den tatsächlichen nächsten Schritt, passenden Status und eine sachgerechte neue Frist umstellen oder bei vollständig erledigtem Vorgang schließen. Keine Parallelaufgabe und keine automatische Drei-Stunden-Eskalation anlegen.
21. **Jede Kundenmail zuerst an Georg; nach eindeutigem Okay den Kundenentwurf direkt anlegen.** Jede neu verfasste E-Mail an einen externen Kunden muss grundsätzlich zuerst als interner Prüfentwurf an `g.schroejahr@rs-led.de` gehen. Vor Georgs Freigabe der exakten Fassung darf kein an den Kunden adressierter Entwurf angelegt werden. Antwortet Georg auf die exakte Prüffassung eindeutig und ohne Änderungswunsch mit `passt`, `ok`, `okay` oder einer gleichbedeutenden Freigabe, den kundenadressierten Entwurf unmittelbar ohne weitere Rückfrage an den Nutzer anlegen, Georg in BCC setzen und den Entwurf vollständig prüfen. Dies ist kein tatsächlicher Versand. Fordert Georg Änderungen oder wird der Text anschließend anderweitig inhaltlich geändert, muss die geänderte Fassung erneut zuerst an Georg. Für gleichlautende Serien- und Nachfassmails gilt ausschließlich die enge Ausnahme in Grundsatz 28.
...
32. **Nach der ersten Nachfassmail exakt 14 Tage bis zur letzten Nachfrage warten und danach schließen.** Sobald der tatsächliche Versand der ersten Nachfassmail im Gesendet-Ordner vollständig verifiziert ist, dieselbe bestehende Twenty-Aufgabe sofort auf exakt 14 Kalendertage nach dem Versandzeitpunkt terminieren. Liegt dann weiterhin keine Kundenantwort vor, eine kurze letztmalige Nachfrage zuerst gemäß Georg-Prüfprozess freigeben lassen. Sie fragt sinngemäß nur letztmalig nach, ob die bisherige Nachricht beziehungsweise das Angebot vollständig angekommen ist, und weist darauf hin, dass sich der Interessent bei weiteren Fragen gerne melden kann. Keine neuen technischen oder kaufmännischen Punkte und keinen Entscheidungsdruck aufnehmen. Erst nach dem tatsächlichen, vollständig verifizierten Versand dieser letzten Nachfrage die bestehende Aufgabe schließen und die Opportunity auf verloren setzen. Ein Entwurf oder eine Freigabe genügt nicht. Bei einer Antwort vor der letzten Nachfrage diese nicht versenden; bei einer späteren Antwort die Opportunity wieder öffnen und einen neuen Task mit Bearbeiter und Frist anlegen. Die Regel gilt für RS-LED-Kundennachfassmails, nicht schematisch für Lieferanten-RFQs, beantwortete Vorgänge oder ausdrücklich anders vereinbarte Kundentermine.
...
- Bleibt nach einem verifizierten Prüfversand an Georg dieselbe Christian zugewiesene Aufgabe als `TODO` mit dem Titelpräfix `Georg-Antwort offen ·` und dem Versandzeitpunkt als Frist in Christians Dashboard sichtbar, solange keine eindeutige Antwort verifiziert ist, und wurde auf eine Parallelaufgabe sowie eine automatische Drei-Stunden-Eskalation verzichtet?
...
- Wurde vor dem Anlegen eines kundenadressierten Entwurfs Georgs Freigabe der exakten Fassung nachgewiesen oder eine gültige Nutzerbestätigung gemäß Serienmail-Ausnahme dokumentiert?
...
- Wurde nach dem verifizierten Versand einer ersten Nachfassmail dieselbe bestehende Aufgabe exakt 14 Kalendertage nach dem Versandzeitpunkt terminiert? Wird bei weiterhin ausbleibender Antwort nur eine kurze letztmalige Nachfrage nach Georg-Prüfung vorbereitet und werden Aufgabe sowie Opportunity erst nach deren tatsächlich verifiziertem Kundenversand geschlossen beziehungsweise auf verloren gesetzt?
